...
let _defineProperty2 = require('babel-runtime/helpers/defineProperty');

let _defineProperty3 = _interopRequireDefault(_defineProperty2);

let _keys = require('babel-runtime/core-js/object/keys');

let _keys2 = _interopRequireDefault(_keys);

let _assign = require('babel-runtime/core-js/object/assign');

let _assign2 = _interopRequireDefault(_assign);

let _typeof2 = require('babel-runtime/helpers/typeof');
...
let _deepDefaults = require('deep-defaults');

let _deepDefaults2 = _interopRequireDefault(_deepDefaults);

let _executeOperation = require('./logic/execute-operation');

let _executeOperation2 = _interopRequireDefault(_executeOperation);

let _font = require('./logic/font');

let Font = _interopRequireWildcard(_font);

let _defaultOptions = require('./default-options');

let _defaultOptions2 = _interopRequireDefault(_defaultOptions);

let _list = require('./logic/list');

let List = _interopRequireWildcard(_list);
 ...
 */
(function () {
    let CommonmarkPDFKitRenderer = function () {
        ...
        (0, _createClass3['default'])(CommonmarkPDFKitRenderer, [{
            ...
            value: function () {
                function operations(nodeTree) {
                    ...
                    let PDFOperationPropertyStack = function () {
                        ...
                        (0, _createClass3['default'])(PDFOperationPropertyStack, [{
                            ...
                            value: function () {
                                function get() {
                                    ...
                                    let stackUnion = this.stack.reduce(function (reduced, current) {
                    ...
                    let stack = new PDFOperationPropertyStack();
                    ...
                    let lastOperationWith = function () {
                    ...
                    let mapListTypes = function () {
                        function mapListTypes(type) {
                            let defaultListStyleType = 'disc';
                            switch (type) {
                                case 'bullet':
                                    return 'disc';
                                case 'ordered':
                    ...
                    while (event = walker.next()) {
                        ...
                        let moveDown = function () {
                            function moveDown() {
                                let moveDownIsAllowed = stack.getValue('listDepth') === 0;
                        ...
                        switch (node.type) {
                            ...
                            case 'softbreak': {
                                if (event.entering) {
                                    let previousText = lastOperationWith('text');
                            ...
                            case 'emph': {
                                if (event.entering) {
                                    let nextFont = stack.getValue('font') === 'bold' ? 'bold-italic' : 'italic';
                            ...
                            case 'strong': {
                                if (event.entering) {
                                    let _nextFont = stack.getValue('font') === 'italic' ? 'bold-italic' : 'bold';
                            ...
                            case 'item': {
                                if (event.entering) {
                                    ...
                                    let listItemIndex = stack.getIncValue('listItemIndex');
                            ...
                            case 'heading': {
                                if (event.entering) {
                                    ...
                                    let _previousText = lastOperationWith('text');
                                    if (_previousText) {
                    ...
                    let removeRedundancies = function () {
                        function removeRedundancies(operation, index, operations) {

                            let previousState = operations.slice(0, index).reduce(function (reduced, current) {
                                return (0, _assign2['default'])(reduced, current);
                            }, {});
                            ...
                            let PROPERTY_RULES = {
                                ...
                                font: function () {
                                    function font(current, previous) {
                                        let hasChanged = current.font !== previous.font;
                                        return (
                            ...
                            let propertiesToKeepEvenIfRedundant = [
                                //'continued', 'link', 'underline'
                            ];

                            if (previousState) {

                                let properties = (0, _keys2['default'])(operation);
                                let propertiesToKeep = properties.filter(function (property) {

                                    let keepAccordingToPropertyRules = PROPERTY_RULES.hasOwnProperty(property) && PROPERTY_RULES[property](operation, previousState);
                                ...
                                let reducedOperation = propertiesToKeep.reduce(function (reduced, property) {
            ...
            value: function () {
                function dimensionsOfMarkdown(doc, nodeTree, pdfkitOptions) {
                    ...
                    let operations = this.operations(nodeTree);

                    let targetWidth = pdfkitOptions && pdfkitOptions.width || doc.page && doc.page.width - doc.page.margins.left - doc.page.margins.right;

                    let initialPosition = {x: doc.x, y: doc.y};
                    let dimensions = (0, _assign2['default'])({}, initialPosition, {
                        w: targetWidth,
                        h: 0
                    });
                    ...
                    operations.forEach(function (op) {
                        ...
                        // change the font
                        if (op.font) {
                            let resolvedFont = Font.forInternalName(op.font, _this.options);
                        ...
                        if (op.text) {
                            ...
                            if (!op.continued) {
                                ...
                                let textOptions = (0, _assign2['default'])({}, pdfkitOptions);
                                ...
                                let delta = doc.heightOfString(continuousText, textOptions);

                                heightChange += delta;

                                // for each linebreak, we need to subtract one currentLineHeight
                                let numLinebreaks = ((continuousText || '').match(/\n/g) || []).length;
                                if (numLinebreaks > 0) {
                                    currentLineHeight = doc._font.lineHeight(currentFontSize, true);
    ...
    module.exports = CommonmarkPDFKitRenderer;
